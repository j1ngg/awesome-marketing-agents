```
                    ___  ___  ___  ___  ___  ___  ___  ___  ___  ___
                   |   \/   ||   \/   ||   \/   ||   \/   ||   \/   |
  __ ___      _____| |\  /| || |\  /| || |\  /| || |\  /| || |\  /| |
 / _` \ \ /\ / / _ \ | \/ | || | \/ | || | \/ | || | \/ | || | \/ | |
| (_| |\ V  V /  __/ |    | || |    | || |    | || |    | || |    | |
 \__,_| \_/\_/ \___|_|    |_||_|    |_||_|    |_||_|    |_||_|    |_|

  m a r k e t i n g   a g e n t s   &   s k i l l s
```

[![Awesome](https://awesome.re/badge.svg)](https://awesome.re)

A curated list of marketing AI agent skills, MCP servers, and open source frameworks for building autonomous marketing workflows.

---

## Contents

| Section | What's inside | Count |
|---------|--------------|-------|
| [Agent Skills](#-agent-skills) | Prompts, workflows, and skill files for marketing agents | 16 |
| [MCP Servers](#-mcp-servers) | Open source MCP servers for marketing platforms | 5 |
| [Frameworks](#-open-source-frameworks) | Agent frameworks for building custom marketing agents | 4 |

---

## 🎯 Agent Skills

Collections of instructions, prompts, and workflows that give AI agents specific marketing capabilities.

### Multi-Skill Collections

*   [ericosiu/ai-marketing-skills](https://github.com/ericosiu/ai-marketing-skills) - 144 open source AI marketing skills covering growth experiments, sales pipeline, content ops, outbound, SEO, and finance automation.
*   [aitytech/agentkits-marketing](https://github.com/aitytech/agentkits-marketing) - Enterprise marketing automation with 18 specialized agents, 93 slash commands, and 28 skills for Claude Code, Cursor, and GitHub Copilot.
*   [kostja94/marketing-skills](https://github.com/kostja94/marketing-skills) - 160+ open source skills for SEO, content, paid ads, channels, and strategies.
*   [coreyhaines31/marketingskills](https://github.com/coreyhaines31/marketingskills) - AI agent skills for CRO and marketing tasks, built for technical marketers and founders.
*   [whyashthakker/agent-skills-marketing](https://github.com/whyashthakker/agent-skills-marketing) - Reusable library of agent skills for marketing, creator operations, SEO, GEO, lifecycle, and campaign execution.

### Research & Intelligence

*   [mvanhorn/last30days-skill](https://github.com/mvanhorn/last30days-skill) - AI agent skill that researches any topic across Reddit, X, YouTube, HN, Polymarket, and the web, then synthesizes a grounded summary.
*   [j1ngg/how-they-market](https://github.com/j1ngg/how-they-market) - Claude Code agent that autonomously researches how companies market themselves.
*   [tessak22/rival](https://github.com/tessak22/rival) - Open source competitive intelligence dashboard powered by the Tabstack API.

### GTM & Launch

*   [j1ngg/tech-marketing-framework](https://github.com/j1ngg/tech-marketing-framework) - Developer tools marketing framework for Claude Code and Codex with repeatable skills for content generation, messaging workshops, and autonomous skill optimization.
*   [j1ngg/launch-framework](https://github.com/j1ngg/launch-framework) - Agent skills for product launch planning and execution.
*   [tessak22/quiver](https://github.com/tessak22/quiver) - AI-powered GTM command center for product teams.
*   [tessak22/devtool-gtm-playbook](https://github.com/tessak22/devtool-gtm-playbook) - Dev tool marketing playbook for GTM strategy.
*   [tessak22/DevRel-Campaign-Drive-Awareness-of-Campaign-Template](https://github.com/tessak22/DevRel-Campaign-Drive-Awareness-of-Campaign-Template) - Developer Relations campaign strategy template.

### Growth & Viral

*   [crealwork/go-viral-or-die](https://github.com/crealwork/go-viral-or-die) - Viral marketing skill for Claude Code inspired by Roy Lee (Cluely). Generates audacious, conversion-aware marketing stunts.

### Design & Creative

*   [pbakaus/impeccable](https://github.com/pbakaus/impeccable) - Design language skill with 18 commands and curated anti-patterns that makes AI coding agents better at frontend design. Supports Claude Code, Cursor, Codex, Gemini, and more.

### Meta: Skill Optimization

*   [olelehmann100kMRR/autoresearch-skill](https://github.com/olelehmann100kMRR/autoresearch-skill) - Autonomously optimize any Claude Code skill by running it repeatedly, scoring outputs against binary evals, and mutating the prompt. Based on Karpathy's autoresearch methodology.

---

## 🔌 MCP Servers

Open source Model Context Protocol servers that give AI agents live access to marketing data and platforms.

| Repo | Platform | Stars |
|------|----------|-------|
| [pipeboard-co/meta-ads-mcp](https://github.com/pipeboard-co/meta-ads-mcp) | Meta / Facebook / Instagram Ads | 798 |
| [googleads/google-ads-mcp](https://github.com/googleads/google-ads-mcp) | Google Ads | 389 |
| [cnych/seo-mcp](https://github.com/cnych/seo-mcp) | SEO (Ahrefs data) | 239 |
| [GeLi2001/shopify-mcp](https://github.com/GeLi2001/shopify-mcp) | Shopify | 195 |
| [shinzo-labs/hubspot-mcp](https://github.com/shinzo-labs/hubspot-mcp) | HubSpot CRM | 34 |

---

## 🛠 Open Source Frameworks

Frameworks for building custom marketing AI agents.

| Repo | Description | Stars |
|------|-------------|-------|
| [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent) | The agent that grows with you. Tool use, memory, extensible skills. | 104k |
| [LangGraph](https://langchain-ai.github.io/langgraph/) | Stateful, multi-actor applications with LLMs. | — |
| [CrewAI](https://crewai.com/) | Orchestrate role-playing, autonomous AI agents. | — |
| [AutoGen](https://microsoft.github.io/autogen/) | Microsoft's framework for multi-agent LLM applications. | — |

---

## Contributing

Contributions welcome! Please read the [contribution guidelines](contributing.md) first.

## License

[![CC0](https://mirrors.creativecommons.org/presscover/cc0-88x31.png)](https://creativecommons.org/publicdomain/zero/1.0/)

To the extent possible under law, the contributors have waived all copyright and related or neighboring rights to this work.
